...
def resolve_dataset_layout(dataset: Path, split: str) -> DatasetLayout:
    ...
    split_value = str(config.get(split, f"images/{split}"))
    images_dir = root / split_value
    labels_dir = root / split_value.replace("images", "labels", 1)
    if not images_dir.exists():
        raise FileNotFoundError(f"Images directory not found: {images_dir}")
    if not labels_dir.exists():
        raise FileNotFoundError(f"Labels directory not found: {labels_dir}")
    return DatasetLayout(root=root, images_dir=images_dir, labels_dir=labels_dir, split=split)
...
<head>
  ...
  <style>
    ...
    header {{ position: sticky; top: 0; z-index: 1; background: #191919; padding: 16px 20px; }}
...
<body>
  <header>
    ...
    <div class="meta">
      ...
      images: {len(previews)} | boxes: {total_boxes}<br>
      dataset: {html.escape(str(layout.root))}
    </div>
  </header>
  <main class="grid">
    {cards}
...
def main() -> None:
    ...
    print(f"Annotation gallery: {index_path}")
