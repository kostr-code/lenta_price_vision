...
def is_validation_sample(video_key: str, timestamp_ms: float, val_ratio: float) -> bool:
    ...
    key = f'{video_key}:{int(round(timestamp_ms))}'.encode()
    ...
    for csv_path in csv_files:
        ...
        try:
            for timestamp, group in df.groupby('frame_timestamp'):
                ...
                frame_variants: list[tuple[object, list[list[float]], int, float]] = [
                    (base_frame, boxes, 0, 1.0)
                ]
                ...
                for frame, variant_boxes, offset, _quality in frame_variants:
                    ...
                    name = (
                        f'{video_path.stem}_{int(round(timestamp_ms)):08d}_'
                        f'{offset:+03d}_{frame_id:06d}.jpg'
                    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            'Build YOLO dataset from provided Lenta CSV labels using '
            'competitor-style full-frame propagation.'
        )
    )
    ...
    parser.add_argument(
        '--data-dir',
        default=None,
        help='Папка с подпапками video/csv. Если не указана, используется авто-поиск.',
    )
    parser.add_argument(
        '--out-dir',
        default='runs/datasets/lenta_yolo_full_prop10',
        help='Папка результата YOLO dataset.',
    )
    parser.add_argument(
        '--propagate',
        type=int,
        default=8,
        help='Сколько соседних кадров добавлять в каждую сторону.',
    )
    ...
    parser.add_argument(
        '--split-mode',
        choices=['frame_id_mod', 'timestamp_hash'],
        default='frame_id_mod',
        help=(
            'frame_id_mod ближе к датасету конкурентов; '
            'timestamp_hash честнее разделяет timestamp-группы.'
        ),
    )
    parser.add_argument(
        '--no-clean',
        action='store_true',
        help='Не удалять старую папку out-dir перед сборкой.',
    )
