...
def suppress_code_artifacts(image_bgr: np.ndarray) -> np.ndarray:
    ...
    texture = cv2.morphologyEx(
        texture, cv2.MORPH_CLOSE, cv2.getStructuringElement(cv2.MORPH_RECT, (5, 5))
    )
...
def ocr_to_lines(ocr_model, image_bgr: np.ndarray) -> list[OCRLine]:
    """Run PaddleOCR on a single image, return List[OCRLine]. No preprocessing."""
    try:
        result = ocr_model.ocr(image_bgr, cls=True)
    except TypeError:
        result = ocr_model.ocr(image_bgr)
    return _paddle_result_to_lines(result)


def ocr_simple(ocr_model, image_bgr: np.ndarray, do_sharpen: bool = True) -> list[OCRLine]:
    """
    One-pass OCR on the full crop with optional light sharpening.

    This is the recommended mode: equivalent to the old ocr_test.py approach.
    No zonal splitting, no CLAHE, no artifact masking.
    """
    img = sharpen(image_bgr) if do_sharpen else image_bgr
    return ocr_to_lines(ocr_model, img)
